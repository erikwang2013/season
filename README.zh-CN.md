...
- 南半球国家与地区已内置映射（澳大利亚 AU、阿根廷 AR、新西兰 NZ、巴西 BR、智利 CL 等），其余按北半球处理。
- **跨赤道国家按“人口多数在哪一侧”判定**，不看国土面积：印尼 ID（雅加达 6.2°S，爪哇岛 1.5 亿人）、肯尼亚 KE（内罗毕 1.3°S，北部干旱人稀）、刚果（布）CG（布拉柴维尔 4.3°S）、厄瓜多尔 EC（基多 0.2°S）算南半球；加蓬 GA（利伯维尔 0.4°N）、乌干达 UG、赤道几内亚 GQ、圣多美 ST、索马里 SO、基里巴斯 KI（塔拉瓦 1.3°N）算北半球。
- 判定规则与回归用例见 `tests/CountrySeasonTest.php`；如需调整，在 `SOUTH_HEMISPHERE_CODES` 增删一个键值对即可。
